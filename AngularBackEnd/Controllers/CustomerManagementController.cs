...
namespace JeeBeginner.Controllers
{
    ...
    public class CustomerManagementController : ControllerBase
    {
        ...
        [HttpGet("GetLoList")]
        public async Task<object> GetLoList(
     [FromQuery] int page = 1,
     [FromQuery] int pageSize = 10,
     [FromQuery] string sortColumn = "IdLo",
     ...
     [FromQuery] string search = ""
 )
        {
            try
            {
                var (items, totalCount) = await _service.GetLoList(page, pageSize, sortColumn, sortDirection, search);

                if (items is null || !items.Any()) return JsonResultCommon.KhongTonTai();

                var result = new
                {
                    data = items,
                    panigator = new { TotalCount = totalCount }
                };
