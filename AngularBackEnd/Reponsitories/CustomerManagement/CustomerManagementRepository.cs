...
using JeeBeginner.Models.KhoManagement;
...
namespace JeeBeginner.Reponsitories.CustomerManagement
{
    public class CustomerManagementRepository : ICustomerManagementRepository
    {
        ...
        public async Task<(IEnumerable<LoModel> items, int totalCount)> GetLoList(int page, int pageSize, string sortColumn, string sortDirection, string search)
        {
            string whereStr = "1=1";
            if (!string.IsNullOrEmpty(search))
            {
                whereStr += $" AND (MaLo LIKE N'%{search}%' OR TenLo LIKE N'%{search}%')";
            }
            ...
            string safeSortDir = "desc".Equals(sortDirection, StringComparison.OrdinalIgnoreCase) ? "DESC" : "ASC";
            string orderByStr = $"{safeSortColumn} {safeSortDir}";
    ...
    ";
            ...
            string sqlCount = $@"SELECT COUNT(*) AS TotalCount FROM DM_LO WHERE {whereStr};";

            using (DpsConnection cnn = new DpsConnection(_connectionString))
            {
                // 1️⃣ Lấy dữ liệu chính
                DataTable dt = await cnn.CreateDataTableAsync(sqlData);

                // 2️⃣ Lấy tổng số bản ghi
                DataTable dtCount = await cnn.CreateDataTableAsync(sqlCount);
                int totalCount = 0;
                ...
                var items = dt.AsEnumerable().Select(row => new LoModel
                {
                    IdLo = Int32.Parse(row["IdLo"].ToString()),
                    MaLo = row["MaLo"].ToString(),
                    TenLo = row["TenLo"].ToString(),
                    NgayNhap = row["NgayNhap"] != DBNull.Value ? ((DateTime)row["NgayNhap"]).ToString("dd/MM/yyyy HH:mm:ss") : "",
                    NgaySanXuat = row["NgaySanXuat"] != DBNull.Value ? ((DateTime)row["NgaySanXuat"]).ToString("dd/MM/yyyy HH:mm:ss") : "",
                    HanSuDung = row["HanSuDung"] != DBNull.Value ? ((DateTime)row["HanSuDung"]).ToString("dd/MM/yyyy HH:mm:ss") : "",
